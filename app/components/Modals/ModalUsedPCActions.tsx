...
export const ModalUsedPCActions = (props: ModalPCActionsProps) => {
  const { isVisible, setIsVisible, pcData, willRefresh } = props
  ...
  const customerOptionsList = [
    ...
    {
      iconName: "package-dependents",
      iconType: "octicon",
      title: "Sacar equipo",
      onPress: async () => {
        Vibration.vibrate(1000)
        let response = await api.post(`${apiRoutes.usedPC.checkOut}/${pcData._id}`)
    ...
    },
    {
      ...
      onPress: async () => {
        Vibration.vibrate(1000)
        let response = await api.delete(`${apiRoutes.usedPC.deleteUsedPC}/${pcData._id}`)
      ...
      <View style={$centeredView}>
        <View style={$modalView}>
          ...
          <View style={$headerContainer}>
            <Text style={$header} text={`${pcData.brand} ${pcData.model}`} />
          </View>
