...
import { Text, PCCard, PCRegisterCard } from "../components"
...
export const NewPCRegisterInquiryScreen: FC<StackScreenProps<AppStackParamList, "NewPCInquiry">> =
  observer(({ navigation, route }) => {
    ...
    const getDataRequests = async () => {
      try {
        ...
        let responseNewPC = await api.get(apiRoutes.newPCRegister.getNewPCsRegister)

        if (responseNewPC.status === 200) {
          setPCRegisterData(responseNewPC.data.newPCsRegister as PCRegister[])
          setFilteredPCRegisterData(responseNewPC.data.newPCsRegister as PCRegister[])
          ...
          >
            ...
            {isLoadingData ? (
              ...
              >
                ...
                <Text text="Registro de equipos nuevos" style={$title} />
                ...
                <View style={$cardWrapper}>
                  {filteredPCRegisterData?.map((item, index) => (
                    <PCRegisterCard key={index} card={item} type="new" />
                  ))}
                </View>
              </ScrollView>
            )}
