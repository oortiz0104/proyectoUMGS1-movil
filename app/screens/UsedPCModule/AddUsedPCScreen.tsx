...
export const AddUsedPCScreen: FC<StackScreenProps<AppStackParamList, "AddUsedPC">> = observer(
  ({ navigation, route }) => {
    ...
    const validateForm = () => {
      if (brand === "" || model === "" || serialNumber === "" || ubication.value === null) {
    ...
    const editPC = async () => {
      ...
      const response = await api.put(`${apiRoutes.usedPC.updateUsedPC}/${id}`, {
        brand: brand.trim(),
        model: model.trim(),
    ...
    const submitPC = async () => {
      ...
      const response = await api.post(apiRoutes.usedPC.checkIn, {
        brand: brand.trim(),
        model: model.trim(),
        serialNumber: serialNumber.trim(),
        ubication: ubication.value,
        ...
        >
          ...
          >
            <View style={$containerNavbar}>
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate("UsedPCInquiry", { willRefresh: !route.params?.willRefresh })
                }
                activeOpacity={0.5}
                hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
              >
            ...
            >
              <Text
                text={!editForm ? "Añadir equipo usado" : "Editar equipo usado"}
                style={$title}
              />
