...
export const AddUserScreen: FC<StackScreenProps<AppStackParamList, "AddUser">> = observer(
  ({ navigation, route }) => {
    ...
    const [isModalRolesVisible, setIsModalRolesVisible] = useState(false)
    ...
    const validateForm = () => {
      ...
      if (username.search(patterns.noSpecialCharacters)) {
        showErrorAlertField("Nombre de usuario", "No se permiten caracteres especiales en el campo")

        return
      }

      if (password.search(patterns.password)) {
        Notifier.showNotification({
    ...
    const submitUser = async () => {
      ...
      const response = await api.post(apiRoutes.user.register_OnlyAdmin, {
        name: name,
        surname: surname,
        email: email,
        phone: phone,
      ...
      <>
        ...
        >
          ...
          >
            ...
            >
              <Text text="Registrar usuario" style={$title} />
              ...
              <View style={$rowInput}>
                <View style={{ width: "100%" }}>
                  <Text text="Usuario" style={$inputLabel} />
                  <Input
                    autoComplete="off"
                    keyboardType="default"
                    autoCapitalize="none"
              ...
              <View style={$rowInput}>
                <View style={{ width: "100%" }}>
                  <Text text="Contraseña" style={$inputLabel} />
                  <Input
                    autoComplete="off"
              ...
              <Text text="Rol" style={$inputLabel} />
              <TouchableOpacity
                style={$pickerButton}
                activeOpacity={0.5}
                onPress={() => setIsModalRolesVisible(true)}
              >
                <View
                  style={{
                    display: "flex",
                ...
                >
                  <Text testID="text_card_1" text={role.label.toString()} style={$optionButton} />
                </View>
              ...
              <ModalPicker
                key={"modalRole"}
                title="Roles"
                isVisible={isModalRolesVisible}
                setIsVisible={setIsModalRolesVisible}
                options={roleOptions}
                actualValue={role}
                setActualValue={setRole}
              />
            </ScrollView>
