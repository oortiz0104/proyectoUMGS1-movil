...
import { Text, PCCard, UserCard } from "../../components"
...
export const UserInquiryScreen: FC<StackScreenProps<AppStackParamList, "UsedPCInquiry">> =
  observer(({ navigation, route }) => {
    ...
    const getDataRequests = async () => {
      try {
        ...
        let responseUser = await api.get(apiRoutes.user.getUsers)

        if (responseUser.status === 200) {
          setUsersData(responseUser.data.users as User[])
          setFilteredUsersData(responseUser.data.users as User[])
            ...
            {isLoadingData ? (
              ...
              <>
                ...
                >
                  ...
                  <Text text="Usuarios" style={$title} />

                  <Input
                    autoComplete="off"
                    placeholder="Buscar por nombre, apellido, email, teléfono o usuario"
                  ...
                  <View style={$containerFilter}>
                    <Text
                      text={`${filteredUsersData?.length ?? 0} Resultados`}
                      style={$labelSort}
                    />
                  </View>
                  ...
                  <View style={$cardWrapper}>
                    {filteredUsersData?.map((item, index) => (
                      <UserCard key={index} card={item} willRefresh={route.params?.willRefresh} />
                    ))}
                  </View>
                </ScrollView>
                ...
                <TouchableOpacity
                  ...
                  style={$fab}
                  onPress={() =>
                    navigation.navigate("AddUser", { willRefresh: route.params?.willRefresh })
                  }
                >
